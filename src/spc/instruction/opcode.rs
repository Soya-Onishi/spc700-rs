...
pub const OPCODE_TABLE: [Opcode; 256] = [
  ...
  NOP,   TCALL, SET1, BBS,  OR,  OR,  OR,  OR,  OR,  OR,       OR_C,   ASL, ASL,  PUSH,  TSET1,   BRK, // 0x0X
  BPL,   TCALL, CLR1, BBC,  OR,  OR,  OR,  OR,  OR,  OR,       DECW,   ASL, ASL,   DEC,    CMP,   JMP, // 0x1X
  CLRP,  TCALL, SET1, BBS, AND, AND, AND, AND, AND, AND,   OR_C_NOT,   ROL, ROL,  PUSH,   CBNE,   BRA, // 0x2X
  BMI,   TCALL, CLR1, BBC, AND, AND, AND, AND, AND, AND,       INCW,   ROL, ROL,   INC,    CMP,  CALL, // 0x3X
  SETP,  TCALL, SET1, BBS, EOR, EOR, EOR, EOR, EOR, EOR,      AND_C,   LSR, LSR,  PUSH,  TCLR1, PCALL, // 0x4X
  BVC,   TCALL, CLR1, BBC, EOR, EOR, EOR, EOR, EOR, EOR,       CMPW,   LSR, LSR,   MOV,    CMP,   JMP, // 0x5X
  CLR_C, TCALL, SET1, BBS, CMP, CMP, CMP, CMP, CMP, CMP,  AND_C_NOT,   ROR, ROR,  PUSH,   DBNZ,   RET, // 0x6X
  BVS,   TCALL, CLR1, BBC, CMP, CMP, CMP, CMP, CMP, CMP,       ADDW,   ROR, ROR,   MOV,    CMP,  RETI, // 0x7X
  SET_C, TCALL, SET1, BBS, ADC, ADC, ADC, ADC, ADC, ADC,      EOR_C,   DEC, DEC,   MOV,    POP,   MOV, // 0x8X
  BCC,   TCALL, CLR1, BBC, ADC, ADC, ADC, ADC, ADC, ADC,       SUBW,   DEC, DEC,   MOV,    DIV,   XCN, // 0x9X
  EI,    TCALL, SET1, BBS, SBC, SBC, SBC, SBC, SBC, SBC,   MOV_TO_C,   INC, INC,   CMP,    POP,   MOV, // 0xAX
  BCS,   TCALL, CLR1, BBC, SBC, SBC, SBC, SBC, SBC, SBC,       MOVW,   INC, INC,   MOV,    DAS,   MOV, // 0xBX
  DI,    TCALL, SET1, BBS, MOV, MOV, MOV, MOV, CMP, MOV, MOV_FROM_C,   MOV, MOV,   MOV,    POP,   MUL, // 0xCX
  BNE,   TCALL, CLR1, BBC, MOV, MOV, MOV, MOV, MOV, MOV,       MOVW,   MOV, DEC,   MOV,   CBNE,   DAA, // 0xDX
  CLR_V, TCALL, SET1, BBS, MOV, MOV, MOV, MOV, MOV, MOV,       NOT1,   MOV, MOV, NOT_C,    POP, SLEEP, // 0xEX
